...
export class FixAdminRBACColumns1733000200000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (disabledAtSnakeExists.length > 0 && disabledAtExists.length === 0) {
      ...
      await queryRunner.query(`ALTER TABLE "users" RENAME COLUMN "disabled_at" TO "disabledAt"`);
    } else if (disabledAtExists.length === 0) {
      // Create the column if it doesn't exist
      await queryRunner.query(`ALTER TABLE "users" ADD COLUMN "disabledAt" TIMESTAMP`);
    }
    ...
    if (disabledExists.length === 0) {
      await queryRunner.query(`ALTER TABLE "users" ADD COLUMN "disabled" boolean NOT NULL DEFAULT false`);
    }
    ...
    if (tableExists.length > 0) {
      ...
      for (const col of columnsToFix) {
        ...
        if (snakeExists.length > 0 && camelExists.length === 0) {
          await queryRunner.query(`ALTER TABLE "admin_audits" RENAME COLUMN "${col.snake}" TO "${col.camel}"`);
        }
      }
    } else {
      ...
      // Add indexes
      await queryRunner.query(`CREATE INDEX "IDX_admin_audits_adminId" ON "admin_audits" ("adminId")`);
      await queryRunner.query(`CREATE INDEX "IDX_admin_audits_targetUserId" ON "admin_audits" ("targetUserId")`);
      await queryRunner.query(`CREATE INDEX "IDX_admin_audits_createdAt" ON "admin_audits" ("createdAt")`);
    }
  }
